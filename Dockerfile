...
ENV ANDROID_SDK_MANAGER 
${SDK_HOME}/android-sdk-linux/tools/bin/sdkmanager

ENV ANDROID_SDK_VERSION sdk-tools-linux-3859397.zip
ENV ANDROID_SDK_URL 
https://dl.google.com/android/repository/${ANDROID_SDK_VERSION}
RUN curl -sSL "${ANDROID_SDK_URL}" -o ${ANDROID_SDK_VERSION} \
    && unzip ${ANDROID_SDK_VERSION} -d ${ANDROID_HOME} \
  && rm -rf ${ANDROID_SDK_VERSION}
  
ENV PATH 
${ANDROID_HOME}/tools:${ANDROID_HOME}/tools/bin:$ANDROID_HOME/platform-tools:$PATH
RUN (while sleep 3; do echo "y"; done) | sdkmanager --update && 
(while sleep 3; do echo "y"; done) | sdkmanager --licenses
...
${ANDROID_HOME}/licenses/android-sdk-preview-license && \
    (while sleep 3; do echo "y"; done) | ${ANDROID_SDK_MANAGER}  
${ANDROID_COMPONENTS} \
                            ${GOOGLE_COMPONENTS} \
                            ${CONSTRAINT_LAYOUT}  
...
RUN (while sleep 3; do echo "y"; done) | ${ANDROID_SDK_MANAGER} 
${ANDROID_NDK_COMPONENTS}  

ENV ANDROID_NDK_HOME ${ANDROID_SDK}/ndk-bundle
ENV PATH ${ANDROID_NDK_HOME}:$PATH
RUN apt-get -qq update && \
    apt-get -qqy install python-pip python-setuptools 
